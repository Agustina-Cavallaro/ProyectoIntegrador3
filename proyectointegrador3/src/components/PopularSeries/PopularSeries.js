...
class PopularSeries extends Component {
    ...
    filtrarElementos (busqueda, datos) { 
        //filtra por parecido de titulo y devuelve solo los qie se parecen
    ...
    cargarMas(){
        const options = {
          ...
          fetch(`https://api.themoviedb.org/3/tv/popular?language=en-US&page=${this.state.contador}`, options)
        .then(res => res.json())
        .then((datos)=> {
            this.setState({
